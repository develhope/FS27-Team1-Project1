...
function hobbies(team1) {
  team1.forEach((person) => {
    console.log(`${person.name}'s hobby is ${person.hobby}`);
  });
}
...
//Correctin book name (trying the rebase rebase)
function correctingTitle(team1) {
    for (let i = 0; i < team1.length; i++) {
      if (team1[i].favoriteBook === "The Stone Monkey") {
        team1[i].favoriteBook = "Journey to the West";
        console.log(team1[i]);
      }
    }
  }
